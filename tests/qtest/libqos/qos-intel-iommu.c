                                     ...
                                     uint64_t slptptr)
{
    ...
    if (mode == QVTD_TM_LEGACY_PT) {
        ...
        lo = VTD_CONTEXT_ENTRY_P | VTD_CONTEXT_TT_MULTI_LEVEL |
             (slptptr & VTD_CONTEXT_ENTRY_SSPTPTR);
        hi = ((uint64_t)domain_id << 8) | QVTD_AW_48BIT_ENCODING;
    }
...
uint32_t qvtd_trigger_dma(QVTDTestContext *ctx)
{
    ...
    /* Write physical address low 32 bits */
    qpci_io_writel(ctx->dev, ctx->bar, ITD_REG_DMA_GPA_LO, (uint32_t)pa);

    /* Write physical address high 32 bits */
    qpci_io_writel(ctx->dev, ctx->bar, ITD_REG_DMA_GPA_HI, (uint32_t)(pa >> 32));
